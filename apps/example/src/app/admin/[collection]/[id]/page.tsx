...
export default function EditRecord({ params }: { params: Promise<{ collection: string; id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/${collectionName}/${id}`).then((r) => r.json()).then(setRecord).catch(() => setRecord(null)).finally(() => setLoading(false));
  }, [collectionName, id]);
  ...
  return <CollectionFormPage config={cfg} collection={collection} record={record} isNew={false} role={(session as { role?: string })?.role} />;
}
