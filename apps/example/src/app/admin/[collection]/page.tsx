...
export default function CollectionList({ params }: { params: Promise<{ collection: string }> }) {
  ...
  useEffect(() => {
    const url = search ? `/api/${collectionName}?search=${encodeURIComponent(search)}` : `/api/${collectionName}`;
    fetch(url).then((r) => r.json()).then((result) => setData(result.data ?? [])).catch(() => {}).finally(() => setLoading(false));
  ...
  return <CollectionListPage config={cfg} collection={collection} data={data} role={(session as { role?: string })?.role} />;
}
